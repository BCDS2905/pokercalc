 ...
 * Compilar: c++ -O3 -shared -fPIC $(python3-config --includes --ldflags) evaluator.cpp -o evaluator.so
 */
...
// Converte string "As" → inteiro 51
int card_to_int(const std::string& c) {
    int rank = RANKS_STR.find(c[0]);
    int suit = SUITS_STR.find(c[1]);
...
// ─── Bitmasks de sequências ───────────────────────────────
static const uint32_t STRAIGHT_MASKS[] = {
    0b11111,        // A-2-3-4-5 (wheel) → tratado separado
    0b11111 << 1,   // 2-3-4-5-6
    0b11111 << 2,
    0b11111 << 3,
    0b11111 << 4,
...
// Retorna valor de 0-8.000.000 (comparável diretamente)
// Codificação: cat*1000000 + primary*10000 + secondary*100 + kicker

struct HandVal {
    int cat;       // 0-8
    int p1, p2, p3, p4, p5;  // valores primário→kickers
...
HandVal eval5(int c0, int c1, int c2, int c3, int c4) {
    ...
    // Bitmask de ranks
    uint32_t mask = (1<<r[0])|(1<<r[1])|(1<<r[2])|(1<<r[3])|(1<<r[4]);
    bool st = false;
    int st_high = 0;
    if (mask == WHEEL_MASK) { st = true; st_high = 3; }  // A-2-3-4-5, high=5
    else {
        for (int i = 1; i <= 9; i++) {
            if (mask == (uint32_t)(0b11111 << i)) { st = true; st_high = i + 3; break; }
        }
    }
    ...
    int fours=-1, three=-1, pairs[2]={-1,-1}, pair_cnt=0;
    // Percorre de A para baixo para pegar o mais alto primeiro
    for (int rank = 12; rank >= 0; rank--) {
        if      (cnt[rank]==4) fours = rank;
        else if (cnt[rank]==3) three = rank;
        else if (cnt[rank]==2) { if(pair_cnt<2) pairs[pair_cnt++]=rank; }
    }
    ...
    if (fours >= 0) {
        int k = -1;
        for (int rank=12; rank>=0; rank--) if(cnt[rank]==1){k=rank;break;}
        return {7, fours, k, 0, 0, 0};
    }

    // Full House
    if (three >= 0 && pair_cnt > 0) return {6, three, pairs[0], 0, 0, 0};
    ...
    if (three >= 0) {
        int k[2]={-1,-1}, ki=0;
        for(int rank=12;rank>=0;rank--) if(cnt[rank]==1 && ki<2) k[ki++]=rank;
    ...
    if (pair_cnt == 1) {
        int k[3]={-1,-1,-1}, ki=0;
        for(int rank=12;rank>=0;rank--) if(cnt[rank]==1 && ki<3) k[ki++]=rank;
...
// ─── Melhor mão de 7 cartas (C(7,5)=21 combinações) ──────
...
std::vector<double> monte_carlo(
    ...
    // Constrói deck disponível
    std::vector<int> used;
    for (auto& c : hole_str)  used.push_back(card_to_int(c));
    for (auto& c : board_str) used.push_back(card_to_int(c));
    ...
    std::vector<int> hole_i, board_i;
    for (auto& c : hole_str)  hole_i.push_back(card_to_int(c));
    for (auto& c : board_str) board_i.push_back(card_to_int(c));
    ...
    for (int sim = 0; sim < n_sims; sim++) {
        ...
        // Board simulado
        int sb[7]; // hole(2) + board(5)
        int sb_idx = 0;
        for (int c : hole_i)  sb[sb_idx++] = c;
        for (int c : board_i) sb[sb_idx++] = c;
        for (int i = 0; i < need; i++) sb[sb_idx++] = deck[i];

        HandVal mine = best7(sb);
        ...
        for (int j = 0; j < opponents; j++) {
            int opp7[7];
            opp7[0] = deck[opp_start + j*2];
            opp7[1] = deck[opp_start + j*2 + 1];
            // board
            for (int c : board_i) opp7[2 + (&c - board_i.data())] = c;
            // board simulado
            for (int i = 0; i < need; i++) opp7[2 + (int)board_i.size() + i] = deck[i];

            HandVal v = best7(opp7);
...
// ─── Monte Carlo Multi (Confronto) ───────────────────────
std::vector<std::vector<double>> monte_carlo_multi(
    const std::vector<std::vector<std::string>>& hands_str,
    const std::vector<std::string>& board_str,
    int n_sims
) {
    int nh = hands_str.size();
    std::vector<std::vector<int>> hands_i(nh);
    std::vector<int> used, board_i;

    for (auto& c : board_str) { board_i.push_back(card_to_int(c)); used.push_back(card_to_int(c)); }
    for (int i = 0; i < nh; i++)
        for (auto& c : hands_str[i]) { hands_i[i].push_back(card_to_int(c)); used.push_back(card_to_int(c)); }
